  ...
  const [data, setData] = useState([
    {
      role: "Wand",
      people: [
        { name: "Dorian", choice: 1 },
        { name: "Dani", choice: 3 },
      ],
    },
    { role: "Potion", people: [{ name: "Monica", choice: 1 }] },
    { role: "Crystal Ball", people: [{ name: "Nico", choice: 1 }] },
  ]);
  ...
  const groups = teams.map(group => {
    ...
    return (
      <Accordion defaultExpanded>
        ...
        <AccordionDetails>
          <div className="flex flex-col">{roleList}</div>
        </AccordionDetails>
      </Accordion>
    );
  });
        ...
        <div className="flex m-5 justify-between">
          ...
          <div className="flex">
            <div className="my-auto">
              <Button variant="contained" className="w-64 h-64" onClick={sortUsers}>
                Randomly Assign teams
              </Button>
            </div>
          </div>
          <div className="mt-10 w-1/3">{teams === null ? <h1>Hi</h1> : groups}</div>
        </div>
        <div className="flex m-5 justify-between">
          <div className="w-1/3">
